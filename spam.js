...
/*From that array, extract just the customers' contact email addresses and
store them in a new array. You will need a nested array method - meaning one
iteration inside another one - since you need to iterate the entire array
of customers, and then iterate the array of emails for each one.*/

let divEl = document.createElement("section")
divEl.setAttribute("id", "contact")
let ulEl = document.createElement("ul")
ulEl.setAttribute("id", "email")
document.querySelector("body").appendChild(divEl)
document.querySelector("section").appendChild(ulEl)

let spamId = document.querySelector("#contact")

customers.forEach(cust => {
  spamId.innerHTML +=
    `<h2>${cust.first_name} ${cust.last_name}</h2>`
  cust.contacts.email.forEach(email => {
    spamId.innerHTML +=
      `<li>${email}`
  })
})
...
let emails = customers.map(stuff => {
  // console.log(stuff.contacts.email)
})

let lotsOfEmails = []
customers.forEach(contact => {
  contact.contacts.email.map(cust => {
    lotsOfEmails.push(cust)
  })
  // console.log(cust.first_name)
  // console.log(cust.last_name)
})
console.log(lotsOfEmails)